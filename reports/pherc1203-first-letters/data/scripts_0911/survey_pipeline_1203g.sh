...
for k in $(seq 0 $((NB - 1))); do
  ...
  for d in $(./venv/bin/python -c "import json; print(' '.join(w['name'] for w in json.load(open('p1203g_survey/windows.json')) if w['batch'] == $k))"); do
    t0=$(date +%s); S=p1203g_survey/${d}_S109; Q=p1203g_survey/${d}_C29; rm -rf $S $Q; mkdir -p $S $Q
    villa-src/volume-cartographer/build/bin/vc_render_tifxyz --volume $H$F --remote-url $H$F -s p1203g_survey/$d --affine p1203_fine/affine_1203.json \
...
echo "SURVEY1203G_DONE $(date +%H:%M:%S)"
