# JOB B2 (11 Sep): the README's window (1-62) on the same 12 windows as job B. Selection code and seed unchanged.
...
VARS = {"readme": {"CANON_START_LAYER": "1", "CANON_END_LAYER": "63"}}   # job B2: the README window on the SAME 12 windows (same seed, same selection code)
...
def collect(n, v, out, rc):
    ...
    print(n, v, res[n][v], f"[{time.time() - t0:.0f}s]", flush=True); json.dump(res, open("/kaggle/working/readme_summary.json", "w"), indent=1)
...
for n in sorted(res):
    f = res[n].get("readme", {})
    if "ink" in f: print(f"{n:34s} published {100 * f['pub']:5.1f}% | README window 1-62: {100 * f['ink']:5.1f}% (match {f['r']:.2f})")
