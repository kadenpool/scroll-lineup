...
for k in $(seq 0 $((NB - 1))); do
  ...
  echo "{\"title\": \"p1203 survey b$k\", \"id\": \"kadenbrodie/p1203-survey-b$k\", \"licenses\": [{\"name\": \"other\"}]}" > $B/dataset-metadata.json
  timeout 1800 ./kagenv/bin/kaggle datasets create -p $B 2>&1 | tail -1
  s=""; for i in $(seq 1 60); do s=$(./kagenv/bin/kaggle datasets status kadenbrodie/p1203-survey-b$k 2>&1 | tail -1); [ "$s" = "ready" ] && break; sleep 20; done
