...
cd "<run-dir>"; H=https://vesuvius-challenge-open-data.s3.amazonaws.com/; Zv=PHerc1203/volumes/20260319130212-2.403um-0.2m-77keV-masked.zarr/
...
for k in $(seq 0 $((NB - 1))); do
  ...
  echo "{\"title\": \"p1203 survey b$k\", \"id\": \"${KAGGLE_USER}/p1203-survey-b$k\", \"licenses\": [{\"name\": \"other\"}]}" > $B/dataset-metadata.json
  timeout 1800 ./kagenv/bin/kaggle datasets create -p $B 2>&1 | tail -1
  s=""; for i in $(seq 1 60); do s=$(./kagenv/bin/kaggle datasets status ${KAGGLE_USER}/p1203-survey-b$k 2>&1 | tail -1); [ "$s" = "ready" ] && break; sleep 20; done
