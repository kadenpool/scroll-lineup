...
scrolls were run by FrankTheRope**; PHerc0813 and PHerc0211 appear only as catalogue entries.
...
**A correction to our own earlier note.** We had recorded that Jinhojeong tested and rejected both CycleGAN
and entropy-minimisation test-time adaptation "on measured grounds". Only the latter is true. CycleGAN
was rejected without being run, and they say why: "Any letters that come out are unverifiable. I did
...
cannot arise by construction. The surface prediction is a separate second gate TAUIL applies only to
...
Their asymmetry, which we carry with the numbers: **"a low, tight reading is evidence. A high,
...
neighbour"; and PHerc0211's inner wrap `z6112_w020` to `z9120_w020`, in reverse. Their status line:
...
mentions PHerc0846A, so the folder is quoted as published data, not as gmDevi's conclusion.
...
offsets across plus or minus 47.98 um. Of the 160 tiles in their main result, 50, which is 31.2 %,
