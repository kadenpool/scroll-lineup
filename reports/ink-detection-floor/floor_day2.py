...
Input dataset: windows.json and masks.npz (made by prep_day1.py before any run) and villa_vesuvius_src.tar
(villa's `vesuvius` package at d285029ab). Everything else is read from the public bucket: PHerc0139's native 9.362 um
surface volumes (zarr v2, uncompressed 28 x 128 x 128 chunks, fetched by plain HTTP) and the two ink_9um checkpoints
from Hugging Face. All windows are on segments whose native 9.362 um scan is not in ink_9um's training data.

Jobs: each donor window read in place (the reference: the same letters, unmodified); each blank target clean; each
target with the donor's letters planted by two methods at three strengths; beside every plant, a sham of the same
method and strength whose "letters" are cut from blank papyrus; and each target with the donor's whole core swapped
in (the transplant: the same letters, cut at the same core edge, in the same surroundings as the plants). Every job is cut to layers [4, 25), run through villa's
own inference with both checkpoints in both directions, and scored by pixel AUC; the raw maps are kept.

Environment handling follows the Kaggle harness behind the ink-negative-controls report: codecs installed before
tifffile is imported, --no-deps installs, every import exercised before the GPU stage, results.json after every step.
env: FL_INPUT FL_WORK FL_TMP FL_PIP FL_PLANTS (additive,swap) FL_STRENGTHS FL_SELFTEST_ONLY FL_LIMIT_TARGETS FL_NO_INFER
"""
import glob, json, os, re, shutil, ssl, subprocess, sys, tarfile, time
from concurrent.futures import ThreadPoolExecutor

SCRIPT_VERSION = "floor_day2 v2 (25 Sep 2026): the day-1 jobs read by hecate 9.6 um (review 7: fp32, exact 9.6 um grid, inputs pinned), generated from floor_kernel v5 (24 Sep 2026): v4 + G3 needs the planted letters to read at s = 1 (median AUC at least 0.70), checkpoints pinned by revision and sha256, clipping over the model's layers"
T0 = time.time()
...
PLANTS = os.environ.get("FL_PLANTS", "additive,swap").split(",")
...
targets = W["targets"][:LIMIT_TARGETS] if LIMIT_TARGETS else W["targets"]
for t in targets:
    dn = donors[t["donor"]]
    sh = W["shams"][t["sham"]]
    v = read_box(t["prefix"], t["box"])
    ...
    k = t_rstd / dn["rstd"]
    d_shift = t["sheet_layer"] - dn["sheet"]                          # carry the donor's sheet onto the target's
    t_resid = core - background(core, dn["neg"])                      # the swap and the transplant replace this
    r_d = shift_z(dn["resid"], d_shift)
    sv = read_box(sh["prefix"], sh["core"]).astype(np.float32)
    sham_r = shift_z(sv - background(sv, dn["neg"]), t["sheet_layer"] - sh["sheet_layer"])
    k_sham = t_rstd / rstd((sv - background(sv, dn["neg"]))[LAYERS[0]:LAYERS[1]][:, dn["neg"]])
    ...
    meta = {"seg": t["seg"], "donor": t["donor"], "sham_core": sh["core"], "sham_seg": sh["seg"], "k": k,
            "k_sham": k_sham, "depth_shift": d_shift}
    ...
    # the transplant: the donor's whole core swapped in (its letters and the papyrus between them), so the letters
    # are cut at the same core edge and sit in the same surroundings as the plants; G1 compares the plants with it
    name = f"{t['name']}__transplant"
    vol, clipped = plant(v, r_d, 1.0, k, cy0, cx0, np.ones((CORE, CORE), np.float32), "swap", t_resid)
    write_job(name, vol)
    SCORE[name] = (pos, neg, (cy0, cx0))
    RESULTS["jobs"][name] = dict(meta, kind="transplant", strength=1.0, clipped_share=clipped)
    log(f"{t['name']}: donor {t['donor']}, k {k:.3f}, sham k {k_sham:.3f}, depth shift {d_shift:+d}")
save()
...
for tag, ck in ck_paths.items():
    ...
    save()
    log(f"{tag} scored: {len(maps)} maps kept")


# ---------------------------------------------------------------- 7. the pre-registered day-1 gates
def med(xs):
...
J = RESULTS["jobs"]
complete = {tag: all(("forward" in J[n].get("auc", {}).get(tag, {}) and "reverse" in J[n]["auc"][tag]) for n in J)
            for tag in ck_paths}
RESULTS["complete"] = complete
gates, lines = {}, ["| checkpoint | plant | dir | reference | transplant | clean | " + " | ".join(f"{s:g}" for s in STRENGTHS) +
                    " | sham " + " / ".join(f"{s:g}" for s in STRENGTHS) + " | planted - transplant | G0 | G1 | G2 | G3 |",
                    "|---|---|---|---|---|---|" + "---|" * len(STRENGTHS) + "---|---|---|---|---|---|"]
for tag in ck_paths:
    if not complete[tag]:
        lines.append(f"| {tag} | INCOMPLETE: some jobs have no score; no gate is read |")
        continue
    ref = {d: med([J[n]["auc"][tag][d] for n in J if J[n]["kind"] == "reference"]) for d in ("forward", "reverse")}
    primary = max(ref, key=lambda d: ref[d])
    ref_by_donor = {J[n]["donor"]: J[n]["auc"][tag][primary] for n in J if J[n]["kind"] == "reference"}
    clean = med([J[n]["auc"][tag][primary] for n in J if J[n]["kind"] == "clean"])
    trans = {n.split("__")[0]: J[n]["auc"][tag][primary] for n in J if J[n]["kind"] == "transplant"}
    trans_med = med(list(trans.values()))
    context_gap = med([J[n]["auc"][tag][primary] - ref_by_donor[J[n]["donor"]] for n in J if J[n]["kind"] == "transplant"])
    for mode in PLANTS:
        curve = {0.0: clean}
        curve.update({s_: med([J[n]["auc"][tag][primary] for n in J if J[n]["kind"] == "planted"
                               and J[n]["plant"] == mode and J[n]["strength"] == s_]) for s_ in STRENGTHS})
        shams = {s_: med([J[n]["auc"][tag][primary] for n in J if J[n]["kind"] == "sham"
                          and J[n]["plant"] == mode and J[n]["strength"] == s_]) for s_ in STRENGTHS}
        at1 = [n for n in J if J[n]["kind"] == "planted" and J[n]["plant"] == mode and J[n]["strength"] == 1.0]
        paired = med([J[n]["auc"][tag][primary] - trans[n.split("__")[0]] for n in at1])
        vs_ref = med([J[n]["auc"][tag][primary] - ref_by_donor[J[n]["donor"]] for n in at1])
        sham_iqr = {s_: [float(np.percentile(v_, 25)), float(np.percentile(v_, 75))] for s_, v_ in
                    ((s_, [J[n]["auc"][tag][primary] for n in J if J[n]["kind"] == "sham" and J[n]["plant"] == mode
                           and J[n]["strength"] == s_]) for s_ in STRENGTHS)}
        ss = sorted(curve)
        g = {"primary_direction": primary, "reference": ref[primary], "transplant": trans_med, "curve": curve,
             "shams": shams, "sham_interquartile": sham_iqr,
             "planted_minus_transplant_at_1": paired, "planted_minus_reference_at_1": vs_ref,
             "context_gap_transplant_minus_reference": context_gap,
             "G0": ref[primary] >= 0.70,
             "G1": abs(paired) <= 0.05,
             "G2": all(0.40 <= shams[s_] <= 0.60 for s_ in STRENGTHS),
             "G3": (0.40 <= clean <= 0.60) and all(curve[b] >= curve[a] - 0.02 for a, b in zip(ss, ss[1:]))
                   and curve[1.0] >= 0.70}
        g["day1_go"] = bool(g["G0"] and g["G1"] and g["G2"] and g["G3"])
        gates[f"{tag}/{mode}"] = g
        lines.append(f"| {tag} | {mode} | {primary} | {ref[primary]:.3f} | {trans_med:.3f} | " +
                     " | ".join(f"{curve[x]:.3f}" for x in ss) +
                     " | " + " / ".join(f"{shams[s_]:.3f}" for s_ in STRENGTHS) + f" | {paired:+.3f} | " +
                     " | ".join("PASS" if g[k_] else "fail" for k_ in ("G0", "G1", "G2", "G3")) + " |")
RESULTS["gates"] = gates
go = [t for t, g in gates.items() if g["day1_go"]]
carry = "swap" if any(t.endswith("/swap") for t in go) else ("additive" if go else None)
RESULTS["carry_forward_plant"] = carry
lines += ["", "Day-1 go: " + (f"YES ({', '.join(go)}); plant carried forward: {carry}" if go else "NO")]
