"""Ink detection floor, day 4 on Kaggle (PREREG.md, amendment 3): the floor on PHerc0483B's surfaces, in two arms.
...
SCRIPT_VERSION = "floor_day4 v1 (25 Sep 2026): the floor on PHerc0483B in two arms, A at 9.362 um (primary) and B at 8.64 um (PREREG amendment 3), generated by make_day4.py from floor_day3 v2 (24 Sep 2026)"
T0 = time.time()
...
RESULTS = {"script_version": SCRIPT_VERSION, "started_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
           ...
           "arms": ARMS, "arm_b_ratio": R_B, "status": "running", "errors": {}, "jobs": {}}
...
def background(v, weight, sigma=SIGMA):
...
w4 = sorted(glob.glob(f"{ROOT}/**/windows_day4.json", recursive=True))
if not w4:
...
RENDERS = {"a": {}, "b": {}}                                          # amendment 3: both arms of each surface
...
SCORE = {}                     # job -> (positives, negatives, (y0, x0, core px)) on its window; maps kept for the core


def write_job(name, vol28, voxel_um=9.362):
...
def core_offset(rec, sfx=""):
    return rec["core" + sfx][0] - rec["box" + sfx][0], rec["core" + sfx][2] - rec["box" + sfx][2]


donors, donors_b = [], []                                            # arm A (day 3's) and arm B (at 8.64 um)
for i, d in enumerate(W["donors"]):
    ...
    SCORE[name] = (pos_w, neg_w, (oy, ox, CORE))
...
targets = W4["targets"][:LIMIT_TARGETS] if LIMIT_TARGETS else W4["targets"]
...
for tag, ck in ck_paths.items():
    ...
    maps = {}
    for name, (pos, neg, (oy, ox, core_px)) in SCORE.items():
        ...
        for d in ("forward", "reverse"):
            ...
            maps[f"{name}__{d}"] = np.rint(p[oy:oy + core_px, ox:ox + core_px] * 255).astype(np.uint8)   # the raw map, kept
...
# ---------------------------------------------------------------- 7. the day-4 readout (PREREG amendment 3): day 3's per arm
def med(xs):
...
D1 = DAY1["jobs"]
readout, lines = {}, ["| arm | checkpoint | dir | control (largest reference change) | clean | " +
...
for ARM, tag in [(a_, t_) for a_ in ("a", "b") for t_ in ck_paths]:     # amendment 3: arm A (primary), then arm B
    ...
    readout.setdefault(ARM, {})[tag] = {"primary_direction": primary, "control_largest_reference_change": moved,
    ...
    lines.append(f"| {ARM} | {tag} | {primary} | {moved:.4f} ({'ok' if control else 'MOVED: not read'}) | {clean:.3f} | " +
...
J = RESULTS["jobs"]                                                   # every job again
