...
| render-path decomposition, 20 Sep 2026 | the same PHerc0139 w016 surface, five renders differing in one thing each | `downstream/results.json`, arms `aligned_nopool`, `theirmesh`, `official_bicubic` | why the challenge's own input reads 0.912 where this folder's render of the same transform reads 0.857: depth averaging worth nothing (0.916 without it), smooth interpolation worth 0.018 and not significant block by block, and the choice of mesh worth 0.040, which is 72 % of the gap (a quarter of it the grid step, `downstream/grid_vs_transform.json`) |
