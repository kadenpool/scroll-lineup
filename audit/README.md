...
```
python audit/audit_landmarks.py            # about a minute, reads only the public metadata
...
The other 21 sit between 0.0 and 25.3 um, and eleven are under 10. So the four are separated from
...
a 9.362 um scan is about 56 um, so 30 um is half of the window that public measurement covers.

It is a deliberately conservative line drawn from a public number, not a derived constant. Nothing in
the table depends on where it sits.
...
- **Not a claim this tool does better.** On PHerc0332 it does not: our answer sits 104 um from the
  same six points, no better than the 103 um the official matrix manages. That pair is hard for both.
  On PHerc1667 1.129 to 2.399 ours reproduces the six landmarks to 6.3 um, which is one pair and not
  a generalisation.
