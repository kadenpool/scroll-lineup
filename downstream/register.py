...
Steps, each checked before the next:
  ...
  3. write labels_on_render.npz with V, I, S and valid built exactly as the 9.362 um control built them.
    ...
    python register.py            # arm c: v6_0139c, tiles_lab/ -> ours_lab.zarr, ours_register.json, ...
    python register.py a          # arm a: v2_0139a, tiles_lab_v2/ -> ours_v2_lab.zarr, ours_v2_register.json, ...
    python register.py o          # the control: the OFFICIAL transform, same scan, same pipeline -> official_*
